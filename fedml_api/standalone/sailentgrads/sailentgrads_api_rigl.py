...
class SailentGradsRiglAPI(object):
    def __init__(self, dataset, device, args, model_trainer, logger):
        ...
        self.init_stat_info()
        #self.weightedaverage=args.weightedaverage
    ...
    def train(self):
        ...
        #Initialize random masks for all clients
        sparsities = self.model_trainer.calculate_sparsities(params,sparse = self.dense_ratio)

        mask_pers_local = [self.model_trainer.init_masks(params, sparsities) for i in range(self.args.client_num_in_total)]
    ...
    #     # Create a CSV file for global model performance
    #     global_model_results_filename = self.args.identity + "global_model_results.csv"
    #     with open(global_model_results_filename, mode='w', newline='') as global_results_file:
    #         global_results_writer = csv.writer(global_results_file)
        
    #     local_model_results_filename = self.args.identity + "local_model_results.csv"
    #     with open(local_model_results_filename, mode='w', newline='') as local_results_file:
    #         local_results_writer = csv.writer(local_results_file)
    ...
    #         # Save global model performance to CSV file
    #         global_results_writer.writerow(["Client {}".format(client_idx), "Round {}".format(round_idx),
    #                                         np.array(g_test_local_metrics['test_correct']) / np.array(g_test_local_metrics['test_total']),
    #                                         g_test_local_metrics['test_loss']])
    ...
    #         local_results_writer.writerow(["Client {}".format(client_idx), "Round {}".format(round_idx),
    #                                         np.array(p_test_local_metrics['test_correct']) / np.array(p_test_local_metrics['test_total']),
    #                                         p_test_local_metrics['test_loss']])
    ...
    def _test_on_all_clients(self, w_global, w_per_mdls, round_idx):
        ...
        with open(global_model_results_filename, mode='a+', newline='') as global_results_file, open(local_model_results_filename, mode='a+', newline='') as local_results_file:
            global_results_writer = csv.writer(global_results_file)
            local_results_writer = csv.writer(local_results_file)
            ...
            for client_idx in range(self.args.client_num_in_total):
                ...
                # Save local model performance to CSV file
                local_results_writer.writerow(["Client {}".format(client_idx), "Round {}".format(round_idx),
                                                np.array(p_test_local_metrics['test_correct']) / np.array(p_test_local_metrics['test_total']),
                                                p_test_local_metrics['test_loss']])
