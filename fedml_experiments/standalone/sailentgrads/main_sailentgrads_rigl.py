...
from fedml_api.standalone.sailentgrads.sailentgrads_api_rigl import SailentGradsRiglAPI
from fedml_api.data_preprocessing.cifar10.data_loader import load_partition_data_cifar10
from fedml_api.data_preprocessing.tiny_imagenet.data_loader import load_partition_data_tiny
from fedml_api.model.cv.resnet import  customized_resnet18, original_resnet18, tiny_resnet18, tiny_resnet50
from fedml_api.standalone.sailentgrads.my_model_trainer_rigl import MyModelTrainerRigL
...
def add_args(parser):
    ...
    parser.add_argument("--weightedaverage", type=bool, default=False)
...
def create_model(args, model_name,class_num):
    ...
    if model_name == "cnn_cifar10":
        ...
        model = tiny_resnet50(class_num=class_num)
...
def custom_model_trainer(args, model, logger):
    return MyModelTrainerRigL(model, args, logger)
...
if __name__ == "__main__":
    ...
    args.identity = "SailentGrads" + "-" + args.dataset + "-" + data_partition + "-" + str(args.weightedaverage)
    ...
    # create model.
    if args.dataset=="cifar10" and args.partition_method=="weighted":
        logger.info("############Printing probabilities of clients for weighted averaging#############")
    ...
    logger.info(model)
    SailentGradsAPI = SailentGradsRiglAPI(dataset, device, args, model_trainer, logger)
